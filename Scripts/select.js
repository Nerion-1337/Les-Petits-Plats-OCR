  ...
  selectLI.forEach((li) => {
    li.addEventListener("click", () => {
      ...
      if (parentClass == "ingredients_ul") {
        const ClassBlue = "ingredients_li";
        ...
        const ClassOrange = "ustensils_li";
...
function ExecutSelect() {
  ...
  const span_ingedients = Array.from(document.querySelectorAll(".select .ingredients_li")).map((span) => span.textContent.toLowerCase().trim());
  const span_appareils = Array.from(document.querySelectorAll(".select .appareils_li")).map((span) => span.textContent.toLowerCase().trim());
  const span_ustensils = Array.from(document.querySelectorAll(".select .ustensils_li")).map((span) => span.textContent.toLowerCase().trim());
